    ...
    <main class="main-content">
      ...
      <div class="creation-form">
        ...
        <form @submit.prevent="submitForm">
          ...
          <div class="form-group">
            <label>Lieu (sélection) *</label>
            <select v-model="formData.lieuId" @change="onLieuChange" required>
              <option value="">Sélectionnez un lieu</option>
              <option v-for="lieu in lieuxFiltrees" :key="lieu.id" :value="lieu.id">
                {{ lieu.nomLieu }} - {{ lieu.projetTitre }}
              </option>
            </select>
          </div>
...
export default {
  ...
  methods: {
    ...
    editPlateau(plateau) {
      ...
      this.filteredLieux = this.lieux.filter(lieu => lieu.projetId === parseInt(this.formData.projetId));
    ...
    onLieuChange() {
      this.lieuSearch = this.getLieuName(this.formData.lieuId);
    },
    goBack() {
      this.$router.go(-1);    
    },
